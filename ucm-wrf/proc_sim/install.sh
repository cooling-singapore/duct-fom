...
if [ "$1" == "nscc" ]; then
  ...
  conda install --yes -c conda-forge rasterio==1.3.10 wrf-python==1.3.4.1 numpy==1.26.4 h5py==3.11.0 pydantic==1.10.14 netCDF4==1.7.1
  ...
  echo "Check if mWRF_SG is already installed"
  WRF_EXE="$HOME/mWRF_SG/main/real.exe"
  if [ -f $WRF_EXE ];then
    echo "Looks like mWRF_SG is already installed. Skipping install."
  ...
  conda create --yes --prefix $WD_PATH/venv python=3.10
  conda activate $WD_PATH/venv
  conda install --yes -c conda-forge rasterio wrf-python numpy h5py pydantic scipy
